...
- [x] Add routing helper command round-trip tests. `cli_status` now covers circuit breaker config/health/reset/stats plus `routing-config global`, `set-global`, `app`, `set-app`, failover queue add/remove/list, `set-auto-failover`, rectifier/optimizer get/set, global outbound proxy get/set/clear, and `routing-runtime status` through the JSON CLI path.
- [x] Mark unsupported or remote-adapted routing subfeatures explicitly. Remote routing keeps the local accordion/component structure while using remote labels for the runtime service and app routing, capability gates for unsupported helper versions, and remote-specific hints only inside the matching local sections.
