...
pub fn build_helper_install_args_with_source(
    ...
    let command = format!(
        concat!(
            ...
            "printf '%s\\n' \"$status_json\" | grep -q '\"settings\"' && ",
            "printf '%s\\n' \"$status_json\" | grep -q '\"plugin\"' && ",
            "printf '%s\\n' \"$status_json\" | grep -q '\"session\"' && return 0; ",
            "echo 'cc-switch-remote helper is missing required capabilities; install a helper build that includes providers, openclaw, mcp, prompts, skills, sessions, hermes-memory, import-export, tools, settings, plugin, and session' >&2; ",
            "return 64; ",
            "}}; ",
