...
pub fn build_helper_install_args_with_source(
    ...
    let command = format!(
        concat!(
            ...
            "for asset_name in cc-switch-cli-{release_tag}-${{asset_os}}-${{asset_arch}} cc-switch-cli-latest-${{asset_os}}-${{asset_arch}} cc-switch-cli-{release_tag}-${{asset_os}}-universal cc-switch-cli-latest-${{asset_os}}-universal; do ",
            "helper_tmp=$(mktemp); ",
